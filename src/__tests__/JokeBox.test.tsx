...
test('Test render jokes', async () => {
  ...
  expect(joke_setup.textContent).toBe("why do python programmers wear glasses?")
...
test('Test right and left click', async () => {
  ...
  const joke = getByText("Your momma is so fat, you need to switch to NTFS to store a picture of her.");
  expect(joke.textContent).toBe("Your momma is so fat, you need to switch to NTFS to store a picture of her.")
...
test("Joke renders when changing category", async () => {
  ...
  await userEvent.click(getByRole("button", {name: "Next"}));
  let joke_setup = getByText("Has COVID-19 forced you to wear glasses and a mask at the same time?")
  let joke_delivery = getByText("If so, you may be entitled to condensation.")
  expect(joke_setup.textContent).toBe("Has COVID-19 forced you to wear glasses and a mask at the same time?")
  expect(joke_delivery.textContent).toBe("If so, you may be entitled to condensation.")
  ...
  expect(joke_number.textContent).not.toBe("2 / 10")
  expect(joke_number.textContent).toBe("1 / 10")
  joke_setup = getByText("I just got fired from my job at the keyboard factory.");
  joke_delivery = getByText("They told me I wasn't putting in enough shifts.");
  expect(joke_setup.textContent).toBe("I just got fired from my job at the keyboard factory.")
  expect(joke_delivery.textContent).toBe("They told me I wasn't putting in enough shifts.")
})
...
test("Test if favorite works", async () => {
  ...
  const joke_setup = getByText("I just got fired from my job at the keyboard factory.");
  const joke_delivery = getByText("They told me I wasn't putting in enough shifts.");
  expect(joke_setup.textContent).toBe("I just got fired from my job at the keyboard factory.")
  expect(joke_delivery.textContent).toBe("They told me I wasn't putting in enough shifts.")
